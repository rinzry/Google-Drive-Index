...
const Os = {
  ...
  isMobile:
    /Android|webOS|iPhone|iPad|iPod|iOS|BlackBerry|IEMobile|Opera Mini/i.test(
      navigator.userAgent
    ),
};
  ...
  return decodeURIComponent(
    atob(str)
      ...
      [gdijsorg_0x22bf03(0x16b)](function (_0x1cdc7a) {
        ...
        return (
          "%" +
          ("00" + _0x1cdc7a[_0x416153(0x166)](0x0)[_0x416153(0x164)](0x10))[
            "slice"
          ](-0x2)
        );
      })
...
function nav(path) {
  ...
  var html = "";
  var cur = window.current_drive_order || 0;
  html += `<div class="alert"> <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span> Content is still copying over to the new drive. Please be patient over the next couple of days. </div>`
...
function append_files_to_list(path, files) {
  ...
  for (i in files.sort(function (a, b) {
    ...
    if (item["mimeType"] == "application/vnd.google-apps.folder") {
      ...
      if (
        "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(
          `|${ext}|`
        ) >= 0
      ) {
        ...
        "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${ext}|`) >= 0
      ) {
      ...
      if (data) {
        ...
        if (href.endsWith("/")) {
          ...
          var ehrefurl =
            href
              .replace(new RegExp("#", "g"), "%23")
              .replace(new RegExp("\\?", "g"), "%3F") + "?a=view";
        }
        title = `Result`;
        $("#SearchModelLabel").html(title);
...
function file_video(path) {
  ...
  <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${
	...
	<video id="vplayer" width="100%" height="100%" playsinline controls: ['play-large', 'play', 'progress', 'current-time', 'mute', 'volume', 'settings', 'pip', 'airplay', 'fullscreen']; data-plyr-config="{ "title": "${decodename}"}" data-poster="${poster}" style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">
...
function read(str) {
  ...
  var sa = str[gdijsorg_0x570bf1(0x15d)](""),
    ...
    aj = ja[gdijsorg_0x570bf1(0x159)](0x18)[gdijsorg_0x570bf1(0x155)](
      0x0,
      -0x14
    );
  return aj;
...
function natsort(r) {
  ...
  var e = /^0/,
    ...
    f = String.prototype.toLocaleLowerCase || String.prototype.toLowerCase,
    s = r.desc ? -1 : 1,
    c = -s,
    l = r.insensitive
      ? function (r) {
          return f.call("" + r).replace(n, "");
        }
      : function (r) {
          return ("" + r).replace(n, "");
        };
  ...
  function d(r, a) {
    return (
      ((!r.match(e) || 1 === a) && parseFloat(r)) ||
      r.replace(t, " ").replace(n, "") ||
      0
    );
  }
  return function (r, e) {
    ...
    var u = p(t),
      ...
      v = t.match(i),
      w = n.match(i),
      g = v && w ? parseInt(v[0], 16) : 1 !== u.length && Date.parse(t),
      h =
        v && w
          ? parseInt(w[0], 16)
          : (g && n.match(o) && Date.parse(n)) || null;
    if (h) {
      if (g === h) return 0;
      if (g < h) return c;
      if (g > h) return s;
    ...
    for (
      var m = u.length, x = f.length, N = 0, $ = Math.max(m, x);
      N < $;
      N += 1
    ) {
      var C = d(u[N] || "", m),
        y = d(f[N] || "", x);
      if (isNaN(C) !== isNaN(y)) return isNaN(C) ? s : c;
      if (a.test(C + y) && C.localeCompare) {
        var D = C.localeCompare(y);
        if (D > 0) return s;
        if (D < 0) return c;
        if (N === $ - 1) return 0;
      }
